...
public class PaymentRestController {
	...
	public ResponseEntity<PaymentModel> findById(@PathVariable("paymentId") Long paymentId){
	...
	public ResponseEntity<List<StatementModel>> getPendingStatements(@PathVariable("cardNumber") String cardNumber){
		return ResponseEntity.ok(paymentService.pendingBills(cardNumber));
	}
	
	@PostMapping("/pendingBills/payUsingUPI/{statementId}")
	public ResponseEntity<PaymentModel> paybill(@RequestBody PaymentModel pay,@PathVariable("statementId") Long statementId) throws PaymentException{
	...
	public ResponseEntity<PaymentModel> paybillUsingAccount(@RequestBody PaymentModel pay,@PathVariable("statementId") Long statementId,@PathVariable("accountNumber") String accountNumber) throws PaymentException{
		return ResponseEntity.ok(paymentService.payBill(pay,statementId,accountNumber));
	}
	
	@GetMapping("/paymentHistory/{cardNumber}")
	public ResponseEntity<List<PaymentModel>> paymentHistory(@PathVariable("cardNumber") String cardNumber) throws PaymentException{
